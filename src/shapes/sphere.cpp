...
{
	Point p = hit - position; // position on sphere centered at origin
	double d = -angle * PI / 180;
	p = p * cos (d) + (axis.cross (p))*sin (d) + axis * (axis.dot (p))*(1 - cos (d)); // Rodrigues' rotation formula

	double u = 0.5 + atan2 (p.y, p.x) / (2 * PI);
	double v = 1 - acos (p.z / r) / PI;
