...
Triple refract (Triple const &incident, Triple const &normal, double n_i, double n_t) {
	return 
		n_i * (incident - incident.dot (normal)*normal) / n_t 
		- normal * sqrt (1 - (n_i*n_i*(1 - pow (incident.dot (normal), 2)) / (n_t*n_t)));
}
